...
#           stdout still carries that label. autodream-now only borrows it as a
#           namespace and always adds a per-install hash, so it ignores this
#           status. A caller that would WRITE the plist -
