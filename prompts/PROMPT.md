...
- **Installed skills**: read `<findings-dir>/skills-inventory.txt` — the authoritative active on-disk skill list the runner wrote (one line per skill: name then description; excludes `~/.omp/agent/config.yml` `skills.ignoredSkills` and frontmatter-disabled skills; covers user, agent, opencode, plugin, and OMP managed skill roots). The built-in binary skills are compiled into the omp binary and are NOT in the inventory — reconcile those against the built-in skills your session skill surface exposes (a built-in you can see is by definition active; never flag it missing). Project-local `.claude/skills` are not enumerable from the runner, so absence from the inventory is not absence. Use this to validate `missed_skill` findings (skill exists? trigger matches?).
...
6. Print: `report: <report-path>` (the literal path from line 2) then a 3-line summary (sessions reviewed, findings), then exit.
