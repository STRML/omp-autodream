...
- Skills invoked (top 5 by count) from `skills_invoked`, which the runner now counts mechanically from OMP `skill-prompt` records rather than asking the L1 model to spot them. Report `skills_authored` separately when non-empty and never fold it in: those are `manage_skill` calls that created, updated or deleted a skill, and a session that wrote four skills without running one is a different finding from a session that ignored the inventory. Reading them as one is how the 2026-09-04 report concluded that ~200 installed skills never fire.
...
- **Network health — read this BEFORE diagnosing capacity**: `network_down_seconds` is the time this run spent blocked waiting for a route, and `network_deferred: yes` means a round could not be dispatched at all. A worker that fails with no route dies in seconds and writes nothing, which looks exactly like a worker that could not digest its transcript. On 2026-09-04 that ambiguity produced a report blaming three transcripts for a 90-minute outage; re-running them later against a live network triaged all three in about 75 seconds each. So when `network_down_seconds` is large or `network_deferred` is `yes`, say that first, attribute the failures to the outage, and do not propose a capacity fix on that run's evidence. **Read all four keys, not just the first two:** `network_down_seconds` and `network_deferred` cover L1 only, because `run-stats.txt` is closed before the aggregator runs; `network_down_seconds_l2` and `network_deferred_l2` are appended afterwards and carry the aggregator's own retry waits. An outage that hit only L2 reads `network_deferred: no` and `network_deferred_l2: yes`, so treating the unsuffixed key as the whole answer misses it. When these keys are absent (findings dirs predating this change), say nothing about any of them.
- **Pipeline capacity**: `l1_findings_with_error` counts sessions that ran to completion but returned empty `findings` with an `error` (transcript too big to fit, even after slimming) — a silent failure that `l1_err_files` (crashed-worker `.err` files) does NOT capture. Compute the extraction-failure rate as `l1_findings_with_error / (sessions_triaged - gated)` — gated sessions never reached a model call, so they cannot contribute an extraction failure and must not inflate the denominator. If that rate is a meaningful fraction, call out the extraction-failure rate and propose the next cc-autodream fix (a lower `AUTODREAM_SLIM_BYTES`, or a richer metadata-only fallback pass; the `jq` pre-summarizer that strips tool-result payloads already ships in `bin/slim-transcript.sh`, so do not propose it again). When `oversized_total > 0`, also report `oversized_errored / oversized_total` — this is the issue #12 measurement gate (chunk-summarizing oversized transcripts is BLOCKED pending evidence it's needed), and if that errored share sustains >= 5% across a week, flag that issue #12's gate has opened. A no-route failure can no longer reach `oversized_errored` at all: an outage leaves the session with no findings file rather than a stub, so there is nothing carrying an `error` key for the counter to see, and the run defers instead of publishing. That means a network outage cannot inflate this ratio. It can deflate it, though: `oversized_total` counts every enumerated session over the threshold whether or not a worker ever reached it, while `oversized_errored` can only count sessions that actually ran and failed. A deferred run therefore reports a full denominator against a partial numerator, so its share reads lower than the evidence supports. Read the network keys above first, and do not fold a deferred run's ratio into the trailing-week judgment at all. If `stats_sidecars_unparseable` is non-zero, carry that caveat into this ratio too — some of those sizes were re-measured live rather than read from a sidecar, so note it next to the gate reading rather than presenting the share as clean.
- **Retry/sleep health**: if `l1_rounds_used` approached `l1_rounds_max` or `l1_missing_after_retries > 0`, the run fought the network/sleep — note it (the laptop likely slept mid-run) and whether the report is complete. `l1_missing_after_retries: 0` and `l1_timed_out: 0` do NOT rule the network out: workers that fail instantly for want of a route leave no timeout and, on the last round, leave a metadata stub rather than a missing slot. `network_down_seconds` is the key that answers this; the other two answer a different question.
