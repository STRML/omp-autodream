...
- **Our own label still wins when it is not the default.** A re-install adopts whatever label the previous install used, so a renamed job does not get a second one beside it. That holds only while the rename keeps the substring `autodream`, because discovery is the `*autodream*.plist` glob (`bin/scheduler-label.sh:60`); a job renamed outside it is invisible here and gets orphaned, exactly as it did before this change.
