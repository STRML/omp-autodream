...
The helper prints a label on every path, including the refusal. `install.sh` writes the plist, so it stops on the refusal. `autodream-now.sh` only borrows the namespace, but on the refusal it cannot use plain `<label>.ondemand`: the other install builds that same label, and the `launchctl bootout` before each on-demand run would evict that install's run. Its plist lives in its own `AUTODREAM_DIR`, so no LaunchAgents scan can see it. On exit 3 `autodream-now.sh` appends the first 8 hex digits of a sha1 of its `AUTODREAM_DIR` (Codex review of `1ee66e4`).
